...
## 8. Bootstrap — reproducing the trust layer

The one-time trust and state backend (the things the pipeline depends on but
cannot create for itself — see the rationale at the end of this section) are
captured in an idempotent, parameterised script: **`scripts/bootstrap.ps1`**.
For the currently-running demo environment these steps are already complete; the
script exists so the environment can be rebuilt from nothing, or a fresh one
stood up, without reconstructing commands by hand.

### What the script provisions

1. Required resource-provider registrations (`Microsoft.ContainerService`,
   `ContainerRegistry`, `Network`, `Compute`, `OperationalInsights`,
   `ManagedIdentity`, `Storage`). Terraform's own auto-registration is disabled
   (`infra/providers.tf`), because the deployer identity is RG-scoped and cannot
   register providers — so they are registered here instead.
2. Application resource group (`housing-notes-rg`).
3. User-assigned managed identity (`housing-notes-deployer`) and its GitHub OIDC
   federated credential, scoped to **`main` only**
   (`repo:atirado-snyk/greenfield:ref:refs/heads/main`).
4. Role assignments on the application RG: **`Contributor`** (manage AKS/ACR) and
   **`Role Based Access Control Administrator`** (so Terraform can create the
   `AcrPull` binding — Contributor alone cannot; see §3).
5. State backend: a separate resource group (`housing-notes-tfstate-rg`), a
   storage account (name **generated** unless supplied), and a blob container
   (`tfstate`), plus **`Storage Blob Data Contributor`** for the deployer on it.
6. Optionally (`-SetGitHubSecrets`, requires `gh auth login`) the three GitHub
   Actions secrets.

### Running it

In Azure Cloud Shell (PowerShell), signed in as a user with Owner / User Access
Administrator on the target subscription:

```
./scripts/bootstrap.ps1 -SubscriptionId <sub-id> -SetGitHubSecrets
```

The script is idempotent — re-running detects and reuses existing resources. On
completion it prints the three secret values and the exact Terraform backend
block to wire into `infra/providers.tf` (the generated storage account name in
particular must be copied there). If `-SetGitHubSecrets` is omitted, set the
three secrets manually from the printed values (§4).

### The currently-running environment

The live demo was bootstrapped with these concrete values (recorded here for
reference; a fresh run via the script will differ on the generated storage
account name):

- App RG `housing-notes-rg`; identity `housing-notes-deployer`; federated
  credential `github-main`.
- Roles: `Contributor` + `Role Based Access Control Administrator` on the app RG.
- State: `housing-notes-tfstate-rg` / storage account `hnotestf3909` /
  container `tfstate`; `Storage Blob Data Contributor` on that account.
- GitHub secrets `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`
  set.

### Why the bootstrap is out-of-band

The pipeline authenticates *as* the deployer identity and writes state to the
backend. Neither the identity nor its own state store can be created by the
pipeline that depends on them — so they are provisioned once, by the script
above, and excluded from Terraform's managed scope. Everything *inside* the
application RG (AKS, ACR, the AcrPull binding) is managed by Terraform and
